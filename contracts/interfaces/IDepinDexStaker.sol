...
import 'depin-dex-v3-periphery/contracts/interfaces/IMulticall.sol';

/// @title DepinDex Staker Interface
/// @notice Allows staking nonfungible liquidity tokens in exchange for reward tokens
interface IDepinDexStaker is IERC721Receiver, IMulticall {
    /// @param rewardToken The token being distributed as a reward
    /// @param pool The DepinDex pool
    ...
    /// @notice The DepinDex Factory
    function factory() external view returns (IDepinDexFactory);
    ...
    /// @notice Withdraws a DepinDex LP token `tokenId` from this contract to the recipient `to`
    /// @param tokenId The unique identifier of an DepinDex LP token
    ...
    /// @notice Event emitted when a DepinDex LP token has been staked
    /// @param tokenId The unique identifier of an DepinDex LP token
    /// @param liquidity The amount of liquidity staked
    /// @param incentiveId The incentive in which the token is staking
    event TokenStaked(uint256 indexed tokenId, bytes32 indexed incentiveId, uint128 liquidity);

    /// @notice Event emitted when a DepinDex LP token has been unstaked
    /// @param tokenId The unique identifier of an DepinDex LP token
    /// @param incentiveId The incentive in which the token is staking
